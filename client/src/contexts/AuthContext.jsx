    ...
    if (user) {
      ...
      checkActivityInterval = setInterval(async () => {
        ...
        if (checkActivity()) {
          ...
          console.log(
            "Current user has been idle for too long - log them out for security and to avoid errors from token expiration."
          );
          await logout();
          // Keep notification up without an auto close interval so idle user can see when they get back
          push({
            type: "info",
            message: `You have been logged out due to extended inactivity.\nPlease log in again.`,
          });
        }
      }, interval_ms);
    }
